...
df_mae = bind_rows(
  ...
  pivot_longer(Y_no_model:Y_hat_egoalter_passed, names_to='model', values_to='val') %>%
  mutate(
    model=case_when(
    ...
    sim=factor(sim, levels=c("Independent", "Degree", "Unobserved", "Main", "Sampled"))
  )
...
p4 = df_perf %>%
  ...
  facet_wrap(~metric, nrow=1) +
  lims(y = c(-0.2, 0.05)) +
  labs(x = element_blank(), y='Homophily bias\n(percent)', title='Homophily bias compared to node-level accuracy') +
  lims(x=c(0.72, 0.87)) +
  theme_cowplot() +
  panel_border() +
  theme(
