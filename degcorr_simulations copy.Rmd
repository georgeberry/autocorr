...
```{r}
fn_dgp_corr_indep_nodesamp = function(n_nodes=4000, node_samp_frac=0.2) {
  ...
  g = g %>%
    ...
    mutate(
      ...
      Y_prob_true = 1 / (1 + exp(- 2 * X - log1p(outdeg))),
      outdeg_inv = 1 / outdeg,
...
fn_dgp_corr_indep_edgesamp = function(n_nodes=4000, edge_samp_frac=0.05) {
  # Creates a graph and draws Y = f(X, Z), where X ~ N(0, 1) and
  # Z is the ego-network avg of the observed X
  g = play_barabasi_albert(n_nodes, 0.8, 5)
  g = g %>%
...
```{r}
df_nodesamp_runs = fn_par_bootstrap(fn_oos, 100, fn_dgp_corr_indep_nodesamp) 
write_csv(
  df_nodesamp_runs,
...
```{r}
df_edgesamp_runs = fn_par_bootstrap(fn_oos, 100, fn_dgp_corr_indep_edgesamp)
write_csv(
  df_edgesamp_runs,
...
```{r}
df_nodesamp = read_csv('/Users/georgeberry/Dropbox/project-autocorr/data/nodesamp_indep_runs.csv')
df_edgesamp = read_csv('/Users/georgeberry/Dropbox/project-autocorr/data/edgesamp_indep_runs.csv')
```
